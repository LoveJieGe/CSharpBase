...
namespace Common.Image
{
    ...
    {
        private static Hashtable iamgeFormTable;
        private static string[] imageType = new string[] {".memorybmp",".bmp",".emf",".wmf",".gif",".jpeg"
,".png",".tiff",".exif",".icon" };
        public ImageHelper()
        {
            iamgeFormTable = Hashtable.Synchronized(new Hashtable());
        }
        ...
        ///  <returns>返回缩略图的路径</returns>
        public static string GenerateThumbnail(string originalImagePath, string thumbnailPath, int width, int height, ThumbnailModel mode)
        {
            System.Drawing.Image originalImage = System.Drawing.Image.FromFile(originalImagePath);
            thumbnailPath = GetThumbnailPath(originalImagePath, thumbnailPath);
            return GenerateThumbnail(originalImage, thumbnailPath, width, height, mode);
        }

        /// <summary>
        /// 生成缩略图
        /// </summary>
        /// <param name="stream">原图片的</param>
        /// <param name="thumbnailPath">缩略图路径（物理路径）</param>
        /// <param name="width">缩略图宽度</param>
        /// <param name="height">缩略图高度</param>
        /// <param name="mode">生成缩略图的方式</param>
        /// <returns>返回缩略图的路径</returns>
        public static string GenerateThumbnail(Stream stream, string thumbnailPath, int width, int height, ThumbnailModel mode)
        {
        ...
        /// 
        /// </summary>
        /// <param name="originalImage">原图片对象</param>
        /// <param name="thumbnailPath">缩略图保存的路径（物理路径）</param>
        /// <param name="width">缩略图宽度</param>
        /// <param name="height">缩略图高度</param>
        /// <param name="mode">生成缩略图的方式</param>
        /// <returns>返回缩略图的路径</returns>
        public static string GenerateThumbnail(System.Drawing.Image originalImage, string thumbnailPath, int width, int height, ThumbnailModel mode)
        {
            if (originalImage == null) return string.Empty;
            int towidth = width;
            int toheight = height;
            ...
            switch (mode)
            {
                case ThumbnailModel.WH:  //指定高宽缩放（可能变形）                
                    break;
            ...
            {
                //以jpg格式保存缩略图
                ImageFormat format = GetImageFormatByExt(originalImage, thumbnailPath);
                thumbnailPath = GetThumbnailPath(originalImage, thumbnailPath);
        ...
        /// <summary>
        /// 根据要保存的缩略图的路径的扩展名字获取图片文件的格式，如果没有则使用原图片的图片格式
        /// 否则默认使用ImageFormat.Jpeg
        /// </summary>
        /// <param name="image">原图片对象</param>
        /// <param name="thumbnailPath">要保存的缩略图的路径</param>
        /// <returns>图片文件格式对象,默认是ImageFormat.Jpeg</returns>
        private static ImageFormat GetImageFormatByExt(System.Drawing.Image image,string thumbnailPath)
        {
            ImageFormat format = null;
            if (Path.HasExtension(thumbnailPath))
            {
                FillImageFormats();
                string ext = Path.GetExtension(thumbnailPath);
                if (!string.IsNullOrEmpty(ext) && iamgeFormTable.ContainsKey(ext.ToLower()))
                {
                    format = iamgeFormTable[ext.ToLower()] as ImageFormat;
                }
            }
        ...
        /// 获取缩略图存放的路径
        /// </summary>
        /// <param name="originalImagePath">原图片的路径，在传入的缩略图路径没有扩展名时，使用原图片的扩展名</param>
        /// <returns>图片扩展名，默认是.jpg</returns>
        public static string GetThumbnailPath(System.Drawing.Image image, string thumbnailPath)
        {
            if (Path.HasExtension(thumbnailPath))
            {
                return thumbnailPath;
            ...
            {
                if (iamgeFormTable.Count <= 0)
                {
                    FillImageFormats();
                }
                string newFileName,ext = null;
                foreach (DictionaryEntry pair in iamgeFormTable)
                {
                    ImageFormat format = pair.Value as ImageFormat;
                    if (format != null && format.Guid == image.RawFormat.Guid)
                    {
                        if (pair.Key != null)
                        {
                            ext = pair.Key.ToString();
                        }
                    }
                }
                if (string.IsNullOrEmpty(ext)) ext = ".jpg";
                newFileName = Guid.NewGuid().ToString() + ext;
                return Path.Combine(thumbnailPath, newFileName);
            }
        }
        /// <summary>
        /// 获取缩略图存放的路径
        ...
        /// <param name="originalImagePath">原图片的路径，在传入的缩略图路径没有扩展名时，使用原图片的扩展名</param>
        /// <returns>图片扩展名，默认是.jpg</returns>
        public static string GetThumbnailPath(string originalImagePath, string thumbnailPath)
        {
            if (Path.HasExtension(thumbnailPath))
            {
                return thumbnailPath;
            ...
            else
            {
                ...
                if (string.IsNullOrEmpty(ext)) ext = ".jpg";
                string newFileName = fileName + "_" + DateTime.Now.ToFileTime() + ext;
                return Path.Combine(thumbnailPath, newFileName);
            }
        }
        /// <summary>
        /// 使用反射获取ImageFormat所有的图片类型，并以扩展名和ImageFormat的形式添加到HashTable中
        /// </summary>
        private static void FillImageFormats()
        {
            ...
            foreach (var property in properties)
            {
                ...
                iamgeFormTable.Add(("." + property.Name).ToLower(), format);
            }
            if (!iamgeFormTable.ContainsKey(".jpg"))
            {
                iamgeFormTable.Add(".jpg", ImageFormat.Jpeg);
            }
        }
        #endregion
    ...
    {
        /// <summary>
        /// 使用指定的宽度和高度进行缩略图的缩放（可能变形） 
        /// </summary>
        WH,
        /// <summary>
        /// 按照指定宽度，高度按比例计算得到(与所填的高度可能不一致)
        /// </summary>
        W,
        /// <summary>
        /// 按照指定的高度缩略，宽度按比例计算得到
