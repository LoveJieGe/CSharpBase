...
namespace DesignPattern
{
    public class ListItem
    {
        ...
        public static ListItem FindByValue(ComboBox cmb, object strValue)
        {
        ...
        /// <returns>返回传入的ComboBox中符合条件的第一个ListItem，如果没有找到则返回null.</returns>
        public static ListItem FindByText(ComboBox cmb, string strKey)
        {
            if (cmb.Items.Count < 0) return null;
            for (int i = 0, count = cmb.Items.Count; i < count; i++)
            {
                var item = cmb.Items[i];
                if ((item is ListItem)&&(item as ListItem).Key.Equals(strKey,StringComparison.CurrentCultureIgnoreCase))
                {
